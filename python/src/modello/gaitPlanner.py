    ...
    def stepTrajectory(self, phi, V, angle, Wrot, centerToFoot):
        ...
        if phi <= stepOffset:  # stance phase
            ...
            stepX_long, stepY_long, stepZ_long = self.calculateStance(
                phiStance, V, angle)  # longitudinal step
            stepX_rot, stepY_rot, stepZ_rot = self.calculateStance(
                phiStance, Wrot, circleTrayectory)  # rotational step
#            print(phi,phiStance, stepX_long)
            ...
            stepX_long, stepY_long, stepZ_long = self.calculateBezier_swing(
                phiSwing, V, angle)  # longitudinal step
            stepX_rot, stepY_rot, stepZ_rot = self.calculateBezier_swing(
                phiSwing, Wrot, circleTrayectory)  # rotational step

        if (centerToFoot[1] > 0):  # define the sign for every quadrant
            if (stepX_rot < 0):
                self.alpha = - \
                    np.arctan2(np.sqrt(stepX_rot**2 + stepY_rot**2), r)
            else:
                self.alpha = np.arctan2(
                    np.sqrt(stepX_rot**2 + stepY_rot**2), r)
        else:
            if (stepX_rot < 0):
                self.alpha = np.arctan2(
                    np.sqrt(stepX_rot**2 + stepY_rot**2), r)
    ...
    def loop(self, V, angle, Wrot, T, offset, bodytoFeet_):
        ...
        step_coord = self.stepTrajectory(
            self.phi + offset[1], V, angle, Wrot, np.squeeze(np.asarray(bodytoFeet_[1, :])))  # FL
        ...
        step_coord = self.stepTrajectory(
            self.phi + offset[2], V, angle, Wrot, np.squeeze(np.asarray(bodytoFeet_[2, :])))  # BR
        ...
        step_coord = self.stepTrajectory(
            self.phi + offset[3], V, angle, Wrot, np.squeeze(np.asarray(bodytoFeet_[3, :])))  # BL
