...
class Robot:
    ...
    def Aggiorna(self, schermata1, schermata2):
        ...
        if(schermata2 != None):
            schermata1.Disegna()
        if(schermata2 != None):
            schermata2.AggiornaCoord()
            schermata2.Disegna()
        self.accXY = self.wifi.Comunica(self.angles)

    # fa camminare il robot
    def Cammina(self, schermata2, schermata1):
        ...
        while self.camminando or (self.planner.phi < 0.99 and not (self.planner.phi > 0.499 and self.planner.phi < 0.51)):
            ...
            self.Aggiorna(schermata1, schermata2) #nuovi angoli
        print(self.planner.phi)

    # fa girare il robot
    def Gira(self, schermata2, schermata1):
        self.girando = True
        V = 0.5  # 0.5
        # il ciclo si interrompe solo se il passo è completo
        while self.girando or (self.planner.phi < 0.99 and not (self.planner.phi > 0.499 and self.planner.phi < 0.51)):
            ...
            self.Aggiorna(schermata1, schermata2)
